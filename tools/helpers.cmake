...
macro(cfg_acc_add_enabled cfg_str cfg_json name var)
    cfg_str_add_define(${cfg_str} ${name} "1" "${var}=${${var}}")
    string(JSON ${cfg_json} SET ${${cfg_json}} "${name}" "true")
endmacro()

macro(cfg_acc_add_disabled cfg_str cfg_json name)
    # include in liner files can't use double slash comments
    list(APPEND ${cfg_str} "/* disabled: CONFIG_${name} */")
    string(JSON ${cfg_json} SET ${${cfg_json}} "${name}" "false")
endmacro()

macro(cfg_acc_add_string cfg_str cfg_json name value)
    if("${value}" STREQUAL "")
...
function(config_option optionname configname doc)
    ...
    if(${optionname})
        cfg_acc_add_enabled(local_config_string local_config_json ${configname} ${optionname})
    else()
        cfg_acc_add_disabled(local_config_string local_config_json ${configname})
    endif()
...
macro(config_set optionname configname value)
    ...
    if("${value}" STREQUAL "OFF")
        cfg_acc_add_disabled(configure_string configure_json ${configname})
    else()
        if("${value}" STREQUAL "ON")
            cfg_acc_add_enabled(configure_string configure_json ${configname} ${optionname})
        else()
            # we have to quote ${value} here because it could be empty
            cfg_acc_add_string(configure_string configure_json ${configname} "${value}")
        endif()
    endif()
endmacro(config_set)
...
function(config_string optionname configname doc)
    ...
    if(cfg_tag_option)
        ...
        cfg_acc_add_string(local_config_string local_config_json ${configname} "${quote}@${cfg_tag_option}@${quote}")
    endif()
    set(configure_string "${local_config_string}" PARENT_SCOPE)
    set(configure_json "${local_config_json}" PARENT_SCOPE)
...
function(config_choice optionname configname doc)
    ...
    foreach(i RANGE 3 ${limit})
        ...
        if(valid)
            ...
            if("${${optionname}}" STREQUAL "${option_value}")
                ...
                cfg_acc_add_enabled(local_config_string local_config_json ${option_config} ${option_cache})
                set(found_current ON)
            else()
                set(${option_cache} OFF CACHE INTERNAL "" FORCE)
                cfg_acc_add_disabled(local_config_string local_config_json ${option_config})
            endif()
        else()
    ...
    if(first)
        ...
        cfg_acc_add_string(local_config_string local_config_json ${configname} "@${optionname}@")
        set(configure_string "${local_config_string}" PARENT_SCOPE)
        set(configure_json "${local_config_json}" PARENT_SCOPE)
        ...
        if(NOT found_current)
            ...
            cfg_acc_add_enabled(local_config_string local_config_json ${first_config} ${first_cache})
        endif()
    endif()
...
function(add_config_library prefix configure_template)
    ...
    set_property(TARGET ${prefix}_Config APPEND PROPERTY GENERATED_JSON_FILE "${config_json_file}")
