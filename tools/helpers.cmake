...
function(add_config_library prefix configure_template)
    ...
    string(CONFIGURE "${configure_template}" config_yaml_contents)
    ...
    add_custom_command(
        ...
        COMMAND
            "${PYTHON3}" "${CONFIG_HEADER_GEN_PATH}" "${config_yaml_file}" -o
            "${config_header_file}"
        DEPENDS "${CONFIG_HEADER_GEN_PATH}" "${config_yaml_file}"
        COMMENT "Generating ${config_header_file}"
        VERBATIM
    )

    add_custom_target(${prefix}_Gen DEPENDS "${config_yaml_file}" "${config_header_file}")
    add_library(${prefix}_Config INTERFACE)
    target_include_directories(${prefix}_Config INTERFACE "${config_dir}")
    add_dependencies(${prefix}_Config ${prefix}_Gen ${config_yaml_file} ${config_header_file})
    ...
    set_property(
        TARGET ${prefix}_Gen
        APPEND
        PROPERTY GENERATED_FILES "${config_yaml_file}" "${config_header_file}"
    )
