 ...
 */
BOOT_CODE bool_t __attribute__(( optimize (
    // 0
    "fsfsda",
    "no-auto-inc-dec",
    "no-branch-count-reg",
    "no-combine-stack-adjustments",
    "no-compare-elim",
    "no-cprop-registers",
    "no-dce",
    "no-defer-pop",
    ...
    "no-tree-ccp",
    "no-tree-ch",
    "no-tree-coalesce-vars",
    "no-tree-copy-prop",
    "no-tree-dce",
    "no-tree-dominator-opts",
    "no-tree-dse",
    "no-tree-forwprop",
    "no-tree-fre",
    "no-tree-phiprop",
    ...
    "no-tree-ter",
    "no-unit-at-a-time"
) )) init_freemem(word_t n_available, const p_region_t *available,
                              ...
                              v_region_t it_v_reg, word_t extra_bi_size_bits)
{
    ...
    while (a < n_available && r < n_reserved) {
        /* We take local copies of array entries for use for reading. This works around an issue in
         * the binary correctness proofs. Note however, that the temporaries are reset at the
         * beginning of each loop iteration, so updates to the current region must still write to
         * the array, not the temporary. */
